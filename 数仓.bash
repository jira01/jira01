...
# -- 导出到linux本地
hive -e "use hainiu;select COALESCE(remote_ip, 'ALL'), COALESCE(substring(time_local,1,8),'ALL'), 
			count(*) from nginx_log group by remote_ip, substring(time_local,1,8) with cube;" 
			1>/home/hadoop/report_test/export_data 2>/dev/null 
...
# 教室集群导入
sqoop import \
--connect jdbc:mysql://nn2.hadoop:3306/hainiutest"?useUnicode=true&characterEncoding=UTF-8" \
--username hainiu \
--password 12345678 \
...
# sqoop导出（教室集群执行）是指从大数据集群（HDFS，HIVE，HBASE）向非大数据集群（RDBMS）中传输数据；
        # 使用export关键字。
...
import
--connect
jdbc:mysql://nn2.hadoop:3306/hainiutest?useUnicode=true&characterEncoding=UTF-8
--username
hainiu
--password
12345678
